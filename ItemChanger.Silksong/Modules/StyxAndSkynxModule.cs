...
public class StyxAndSkynxModule : Module, ICumulativeCostModule
{
    ...
    // Time (seconds) required to reduce the RegrowDebt by 1, by growing another Silkeater.
    public long DebtGrowTimeMillis = 180_000;  // 3 minutes.

    // Time (seconds) required to grow Silkeaters normally once the shop is cleaned out.
    public long DefaultGrowTimeMillis = 3_600_000;  // 1 hour.

    // Time (seconds) required to 'grow' placements at the next available farm level, after either rescuing Styx, or delivering the Queen's Egg respectively.
    ...
    {
        clockModule = ItemChangerHost.Singleton.ActiveProfile!.Modules.GetOrAdd<ClockModule>();
        ...
        this.UsePatches(typeof(Patches));
